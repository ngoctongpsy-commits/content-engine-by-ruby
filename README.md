...
- **Video (`video` + `video-production` skill)**: on-demand short-form video - script/storyboard + clip generated via the connected `~~video` MCP (default Higgsfield) at per-platform aspect ratios + captions. Runs on `/make-video`, not auto-scheduled. Generating consumes Higgsfield credits.
